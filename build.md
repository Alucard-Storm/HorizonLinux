...
Boot the generated ISO in QEMU:

```bash
sudo qemu-system-x86_64 -enable-kvm -m 4096 -serial stdio -cdrom /tmp/horizon-linux-build/horizon-linux.x86_64-1.0.0.iso
```
...
- NVIDIA driver setup failures are logged to `/root/horizon-linux-post.log` rather than aborting image creation. After first boot, rebuild the driver and initramfs when necessary:
